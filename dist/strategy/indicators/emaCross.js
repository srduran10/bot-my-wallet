...
exports.detectEmaCross = detectEmaCross;
 ...
 */
function detectEmaCross(prices, shortPeriod = 9, longPeriod = 21) {
    if (!Array.isArray(prices) || prices.length < longPeriod + 2) {
        console.warn('⚠️ Lista de precios insuficiente para cálculo de EMA.');
        return 'HOLD';
    }
    const shortEma = technicalindicators_1.EMA.calculate({ period: shortPeriod, values: prices });
    const longEma = technicalindicators_1.EMA.calculate({ period: longPeriod, values: prices });
